...
async function main() {
  ...
  if (version.version === '2.6.17') pass('version.json', version.version);
  else fail('version.json', JSON.stringify(version));

  const sw = read('sw.js');
  if (sw.includes("const CACHE = 'msb-pro-v2.6.17'")) pass('sw-cache');
  ...
  if (index.includes("const APP_VERSION = '2.6.17'") && index.includes('id="app-version-label">v2.6.17')) {
    pass('index-version');
  ...
  try {
    ...
    if (!fb.hasFn) fail('playtest-feedback-fn', 'openFeedbackPage missing');
      ...
      await fbPage.goto(base + '/feedback.html?v=2.6.17', { waitUntil: 'domcontentloaded', timeout: 30000 });
