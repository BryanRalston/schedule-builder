...
async function main() {
  ...
  const sw = read('sw.js');
  if (sw.includes("const CACHE = 'msb-pro-v2.6.9'")) pass('sw-cache');
  ...
  if (index.includes("const APP_VERSION = '2.6.9'") && index.includes('id="app-version-label">v2.6.9')) {
    pass('index-version');
  ...
  try {
    ...
    if (boot.ver === '2.6.9') pass('live-app-version', boot.ver);
